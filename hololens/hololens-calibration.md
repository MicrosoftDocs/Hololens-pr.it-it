...
description: Informazioni su come calibrare la distanza interpupupry (IPD) per migliorare la qualità degli oggetti visivi nei dispositivi HoloLens.
author: Teresa-Motiv
ms.author: xerxesb
ms.date: 9/13/2019
ms.topic: article
...
Anche se entrambi i dispositivi devono calibrare per la migliore esperienza di visualizzazione degli ologrammi, usano tecniche e tecnologie di calibrazione diverse.  Passare alla [calibrazione HoloLens 2](#calibrating-your-hololens-2) o [a HoloLens (prima generazione).](#calibrating-your-hololens-1st-gen)
...
Durante questo processo verrà visualizzato un set di destinazioni (gemme). È possibile lampeggiare durante la calibrazione, ma provare a concentrarsi sulle gemme invece che su altri oggetti nella stanza.  Concentrarsi sulle gemme consente a HoloLens di conoscere la posizione dell'occhio per eseguire il rendering del mondo olografico.
...
1. Se l'app Impostazioni non è aggiunta a **Start,** selezionare **Tutte le app.**
...
La funzionalità Auto Eye Position (Posizione oculare automatica) comporta alcune modifiche al comportamento del sistema quando un utente non bilanciato posiziona il dispositivo. In questo contesto, un utente noncalibrato fa riferimento a un utente che non ha mai fatto il processo di calibrazione del tracciamento oculare nel dispositivo in precedenza.
...
Per le esperienze che richiedono dati dello sguardo fisso o un posizionamento preciso dell'ologramma, è consigliabile che gli utenti noncalibrati esecutorino la calibrazione del tracciamento oculare. È accessibile dal prompt di calibrazione del tracciamento oculare o avviando l'app Impostazioni dal menu Start e quindi selezionando Calibrazione > sistema > calibrazione **oculare > Esegui** calibrazione oculare.
...
1.  La finestra di dialogo Eye Tracking Calibration Prompt (Richiesta calibrazione tracciamento oculare) fornisce all'utente informazioni dettagliate sul motivo per cui è necessario il tracciamento oculare.
...
Le informazioni di calibrazione vengono archiviate localmente nel dispositivo e non sono associate ad alcuna informazione sull'account. Non è presente alcun record di chi ha usato il dispositivo senza calibrazione. Ciò significa che i nuovi utenti riceveranno la richiesta di calibrare gli oggetti visivi quando usano il dispositivo per la prima volta e gli utenti che hanno scelto esplicitamente la calibrazione in precedenza o se la calibrazione non è riuscita.
...
Le informazioni sulla calibrazione possono sempre essere eliminate dal dispositivo in  >  **Impostazioni Privacy**  >  **Tracciamento oculare.**  
...
1. Selezionare **Impostazioni**  >  **Calibrazione**  >  **sistema**.
1. Disattiva **Quando una nuova persona usa questo HoloLens, chiedi automaticamente di eseguire la calibrazione oculare.**
...
Quando si configura il dispositivo HoloLens (prima generazione), viene richiesto di calibrare gli oggetti visivi dopo l'introduzione di Cortana. È consigliabile completare il passaggio di calibrazione durante questa fase di installazione. È tuttavia possibile ignorarlo attendendo fino a quando Cortana non chiede conferma e quindi pronunciando "Ignora".
...
Se è necessario aggiornare la calibrazione o se un nuovo utente deve regolarla, è possibile eseguire manualmente l'app Calibrazione in qualsiasi momento. L'app Calibrazione viene installata per impostazione predefinita. È possibile accedervi usando il menu **Start o** l'app Impostazioni.

Per usare il menu **Start** per eseguire l'app Calibrazione, seguire questa procedura:

1. Usare il [movimento bloom](hololens1-basic-usage.md) per aprire il menu **Start.**
...
1. Usare il [movimento bloom](hololens1-basic-usage.md) per aprire il menu **Start.**
1. Se **Impostazioni** non è aggiunto a **Start,** selezionare **+** per visualizzare tutte le app.
...
Alcuni visori immersivi offrono la possibilità di personalizzare l'impostazione IPD. Per modificare l'IPD per il visore, aprire l'app Impostazioni e selezionare **Visualizzazione** visore realtà mista  >  e quindi spostare il dispositivo di scorrimento. Le modifiche verranno visualizzati in tempo reale nel visore. Se si conosce l'IPD, magari da una visita all'optometrista, è possibile immetterlo direttamente.

È anche possibile modificare questa impostazione nel PC selezionando **Impostazioni**  >  **Schermo visore realtà**  >  **mista**.

Se il visore non supporta la personalizzazione ipd, questa impostazione verrà disabilitata.
