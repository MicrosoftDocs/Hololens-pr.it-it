...
ms.locfileid: "126032598"
---
# <a name="manage-user-identity-and-login-for-hololens"></a>Gestire l'identità utente e l'account di accesso per HoloLens

> [!NOTE]
> Questo articolo è un riferimento tecnico per professionisti IT e appassionati di tecnologia. Se si stanno cercando istruzioni HoloLens di configurazione, vedere " Configurazione del HoloLens[(prima generazione)](hololens1-start.md)" o " Configurazione del[HoloLens 2](hololens2-start.md)".

## <a name="lets-talk-about-setting-up-user-identity-for-hololens-2"></a>Si parlerà della configurazione dell'identità utente per HoloLens 2

Come altri Windows, HoloLens funziona sempre in un contesto utente. Esiste sempre un'identità utente. HoloLens tratta l'identità quasi nello stesso modo di un Windows 10 dispositivo. L'accesso durante l'installazione crea un profilo utente HoloLens che archivia app e dati. Lo stesso account fornisce anche l'accesso Single Sign-On per le app, ad esempio Edge o Dynamics 365 Remote Assist, usando le API Windows Account Manager. 

HoloLens supporta diversi tipi di identità utente. È possibile scegliere uno qualsiasi di questi tre tipi di account, ma è consigliabile Azure AD per la gestione dei dispositivi. Solo Azure AD account supportano più utenti. 
...
| [Account Microsoft (MSA)](/windows/security/identity-protection/access-control/microsoft-accounts) | 1 | <ul><li>Solo biometria (Iris) &ndash; HoloLens 2</li><li>PIN &ndash; Facoltativo per HoloLens (prima generazione), necessario per HoloLens 2</li><li>Password</li></ul> |
...
Se si usa un account aziendale o aziendale per accedere a HoloLens, HoloLens si registra nell'infrastruttura IT dell'organizzazione. Questa registrazione consente all'amministratore IT di configurare Gestione dispositivi mobili (MDM) per inviare criteri di gruppo HoloLens.
...
### <a name="setting-up-multi-user-support-azure-ad-only"></a>Configurazione del supporto multi-utente (solo Azure AD multi-utente)

HoloLens supporta più utenti dallo stesso tenant Azure AD tenant. Per usare questa funzionalità, è necessario usare un account appartenente all'organizzazione per configurare il dispositivo. Successivamente, altri utenti dello stesso tenant possono accedere al dispositivo dalla schermata di accesso o toccando il riquadro utente nel pannello Start. È possibile accedere a un solo utente alla volta. Quando un utente esegue l'accesso, HoloLens l'utente precedente. 
...
Se [l'app](/uwp/api/Windows.Security.Credentials.UI) usa altri tipi di autenticazione, ad esempio NTLM, Basic o Kerberos, è possibile usare l'interfaccia utente delle credenziali di Windows per raccogliere, elaborare e archiviare le credenziali dell'utente. L'esperienza utente per la raccolta di queste credenziali è simile ad altre interruzioni dell'account basato sul cloud e viene visualizzata come app figlio sopra l'app 2D o sospende brevemente un'app Unity per visualizzare l'interfaccia utente.
...
Windows Hello for Business ( che supporta l'uso di un PIN per l'accesso) è supportato per HoloLens (prima generazione). Per consentire Windows Hello'accesso con PIN aziendale all'HoloLens:
...
- **account Microsoft:** il comportamento del blocco è diverso e consente lo sblocco automatico, ma l'autenticazione di accesso è ancora necessaria al riavvio.
