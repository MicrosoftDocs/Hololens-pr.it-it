...
ms.locfileid: "112924418"
---
# <a name="manage-user-identity-and-sign-in-for-hololens"></a>Gestire l'identità utente e l'accesso per HoloLens

> [!NOTE]
> Questo articolo è un riferimento tecnico per professionisti IT e appassionati di tecnologia. Se si stanno cercando istruzioni per la configurazione di HoloLens, vedere " Configurazione di[HoloLens (prima generazione)](hololens1-start.md)" o " Configurazione del[HoloLens 2](hololens2-start.md)".

Come altri dispositivi Windows, HoloLens opera sempre in un contesto utente. Esiste sempre un'identità utente. HoloLens tratta l'identità quasi nello stesso modo di altri Windows 10 dispositivi. Questo articolo è una guida di riferimento approfondito per l'identità in HoloLens e illustra le differenze tra HoloLens e altri Windows 10 dispositivi.
...
| --- | --- | --- |
| [Azure Active Directory](https://docs.microsoft.com/azure/active-directory/)<sup>1</sup>  | 64 | <ul><li>Provider di credenziali Web di Azure</li><li>Azure Authenticator App</li><li>Biometria (Iris) &ndash; HoloLens 2 solo<sup>2</sup> </li><li>PIN &ndash; facoltativo per HoloLens (prima generazione), necessario per HoloLens 2</li><li>Password</li></ul> |
...
Come Windows in altri dispositivi, l'accesso durante l'installazione crea un profilo utente nel dispositivo. Il profilo utente archivia app e dati. Lo stesso account fornisce anche l'accesso Single Sign-On per le app, ad esempio Edge o Microsoft Store, usando le API di Gestione account di Windows.  
...
HoloLens supporta più utenti dello stesso tenant Azure AD tenant. Per usare questa funzionalità, è necessario usare un account appartenente all'organizzazione per configurare il dispositivo. Successivamente, altri utenti dello stesso tenant possono accedere al dispositivo dalla schermata di accesso o toccando il riquadro utente nel pannello Start. È possibile accedere a un solo utente alla volta. Quando un utente esegue l'accesso, HoloLens esegue la firma dell'utente precedente. Il primo utente del dispositivo è considerato il proprietario del dispositivo, ad eccezione del caso di Azure AD join, per altre informazioni sui [proprietari del dispositivo.](security-adminless-os.md#device-owner)
...
HoloLens 2 supporta l'autenticazione Iris. Iris si basa sulla tecnologia Windows Hello ed è supportata per l'uso sia da Azure Active Directory che dagli account Microsoft. Iris viene implementato come altre tecnologie Windows Hello e ottiene la sicurezza biometrica FAR di 1/100.000.

Per altre informazioni, vedere le specifiche e i [requisiti biometrici Windows Hello](https://docs.microsoft.com/windows-hardware/design/device-experiences/windows-hello-biometric-requirements) informazioni. Altre informazioni su [Windows Hello](https://docs.microsoft.com/windows-hardware/design/device-experiences/windows-hello) e [Windows Hello for Business](https://docs.microsoft.com/windows/security/identity-protection/hello-for-business/hello-identity-verification). 
...
Per altre informazioni sulla protezione delle identità utente e sull'autenticazione, vedere la [documentazione Windows 10 sicurezza e identità.](https://docs.microsoft.com/windows/security/identity-protection/)
