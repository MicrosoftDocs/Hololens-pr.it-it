...
- ./Vendor/MSFT/DeviceStatus/NetworkIdentifiers/*indirizzo mac* della scheda Wi-Fi /SSID
...
Introdotto in Windows Insider build 20348.1405. È stato aggiunto il supporto per Gestione [certificati per](certificate-manager.md) l'uso dei certificati PFX. Quando gli utenti passano **Impostazioni** aggiornano i & di sicurezza e selezionano Installa un certificato, l'interfaccia utente supporta ora  >    >  il file di certificato PFX. 
...
Ci auguriamo che con questa aggiunta più recente di commenti e suggerimenti audio-video sia più facile raccogliere i dati di diagnostica e più rapidamente essere in grado di risolvere i problemi.

### <a name="use-only-private-store-apps-for-microsoft-store"></a>Usare solo app dello Store privato per Microsoft Store

I criteri RequirePrivateStoreOnly sono stati abilitati per HoloLens. Questo criterio consente all'app Microsoft Store essere configurata in modo da visualizzare solo lo store privato configurato per l'organizzazione. Limitazione dell'accesso solo alle app rese disponibili.
