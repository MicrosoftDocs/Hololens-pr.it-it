...
Questa versione più recente è un aggiornamento mensile alla versione 21H1, ma questa volta sono incluse nuove funzionalità. Il numero di build principale rimarrà invariato e Windows aggiornamento indicherà una versione mensile alla versione 21H1 (build 20348). È possibile esaminare il numero di build nella schermata Impostazioni > Informazioni su per verificare di essere nella build più recente disponibile 20348.1432+. Per eseguire l'aggiornamento alla versione più recente, aprire l'app Impostazioni, passare a Aggiorna & Sicurezza e toccare Controlla aggiornamenti. Per altre informazioni su come gestire gli aggiornamenti HoloLens, vedere [Gestire HoloLens aggiornamenti.](hololens-updates.md)

| Funzionalità                 | Descrizione                | Utente o scenario |
|-------------------------|----------------------------|--------------|
| [Spostamento della modalità piattaforma](#moving-platform-mode) | Introduce la versione beta moving platform mode, che, se configurata, consente l'uso di HoloLens 2 su navi di grandi dimensioni con movimento a bassa dinamica. | Tutti |
...
| [Modifiche di CSP per la creazione di report HoloLens dettagli](#csp-changes-for-reporting-hololens-details) | Nuovi CSP per eseguire query sui dati | Amministratori IT    |
...
1. Aprire l Impostazioni app e passare a [Gestione certificati](certificate-manager.md) e applicare il certificato.
...
- Indirizzo *mac*./Vendor/MSFT/DeviceStatus/NetworkIdentifiers/ della Wi-Fi/BSSID
...
> - L'accesso automatico è supportato solo per l'account del servizio AAD utenti.
...
È ora possibile usare WDAC per bloccare l'avvio di app o processi e continuare a usare la propria linea di app. È ora possibile consentirle nei criteri di WDAC. L'uso di questi criteri comporta l'esecuzione di una riga di codice aggiuntiva in PowerShell durante la creazione dei criteri wdac. [Esaminare i passaggi qui.](/mem/intune/configuration/custom-profile-hololens)

#### <a name="overview-to-try-your-own-apps-while-using-wdac-to-block-others"></a>Panoramica per provare le proprie app mentre si usa WDAC per bloccare altri utenti
...
- L'elaborazione del gamepad per le app 2D è stata disabilitata nelle build Insider. Rimuovendo questa funzionalità, le app possono ora usare direttamente le API gamepad e avere accesso all'intero set di controlli e possono essere sviluppate per eseguire altre attività. Gli sviluppatori devono usare le API Gamepad per usare l'input gamepad. Ecco un esempio per la [classe Gamepad (Windows. Gaming.Input) - Windows applicazioni UWP.](/uwp/api/windows.gaming.input.gamepad?view=winrt-20348&preserve-view=true)
...
#### <a name="for-enterprise"></a>Per Enterprise
...
- È stato risolto un problema a causa del quale dopo il primo accesso dell'utente, la Configurazione configurazione utente era terminata negli scenari in cui venivano usate AAD configurazioni in modalità tutto schermo basate su gruppo.
...
- È stato risolto un problema a causa del quale dopo il riavvio del dispositivo, i controller Xbox e Bluetooth le periferiche LE dovevano essere nuovamente associati per la connessione.
- Risolto un problema del codificatore video che potrebbe causare un breve blocco del video in uscita durante una Remote Assist chiamata. Wi-Fi modifiche del driver e del firmware per risolvere "Frammenti e forgiatura" Wi-Fi vulnerabilità.
- Wi-Fi modifiche del driver e del firmware per risolvere "Frammenti e forgiatura" Wi-Fi vulnerabilità.
- Quando si usa la modalità MPM (Moving Platform Mode), "Down" verrà stimato in base alla media della gravità per un breve periodo di tempo. Questo valore sostituisce la vera gravità in modalità piattaforma mobile.
- Correzione del wobble periodico negli ologrammi in modalità 3DoF o durante la perdita del rilevamento.
- Risolve un problema che influisce sugli aggiornamenti alla versione 21H1/21H2 delle versioni precedenti.

## <a name="windows-holographic-version-20h2---october-2021-update"></a>Windows Holographic, versione 20H2 - Aggiornamento di ottobre 2021

- Build 19041.1168

Miglioramenti e correzioni nell'aggiornamento:

- Questo aggiornamento qualitativo mensile non contiene modifiche di primo piano. È quindi necessario provare la build più recente, Windows Holographic, versione 21H2.
...
- Correzioni per risolvere il problema a causa del quale l'ora di sistema potrebbe saltare in modo imprevisto.
...
- È stato risolto un problema per cui Wi-Fi proxy non può essere salvato quando l'interfaccia utente di Wi-Fi properties viene avviata da **Impostazioni > Network & Internet > Status > Properties**.
...
[Nuove impostazioniURI per la visibilità Impostazioni pagina](#new-settings-uris-for-page-settings-visibility) | Oltre 20 nuovi criteri SettingsURIs per Impostazioni/PageVisibilityList. | Amministratore IT |
...
Il nuovo Microsoft Edge ![nuova Microsoft Edge icona.](images/new_edge_logo.png) (rappresentato da un'icona di scorrimento blu e verde) viene aggiunto al menu Start e verrà avviato automaticamente quando si attiva un collegamento Web.

> [!NOTE]
> Quando si avvia per la prima volta la nuova Microsoft Edge in HoloLens 2, le impostazioni e i dati verranno importati dalle versioni Microsoft Edge. Se si continua a usare i Microsoft Edge legacy dopo l'avvio del nuovo Microsoft Edge, i nuovi dati non verranno sincronizzati dal Microsoft Edge legacy al nuovo Microsoft Edge.
...
- L'anteprima della lente di ingrandimento nella tastiera olografica è stata disabilitata per il nuovo Microsoft Edge. Ci auguriamo di riabilitare questa funzionalità in un aggiornamento futuro, una volta che l'ingrandimento funziona correttamente.
- L'audio può essere riprodotto dalla finestra errata del browser se è aperta e attiva un'altra finestra del browser. È possibile risolvere questo problema chiudendo l'altra finestra attiva che non dovrebbe riprodurre audio.
...
**Installazione diretta nel dispositivo (attualmente disponibile solo per i dispositivi non gestiti)**
  ...
  1. Avviare il file con estensione msix scaricato dalla coda di download edge o dalla cartella "Download" del dispositivo (usando Esplora file).
...
### <a name="install-web-apps"></a>Installare app Web
 ...
 > [!Note]
>A Windows [Holographic, versione 21H1,](hololens-release-notes.md#windows-holographic-version-21h1)Office'app Web non verrà più preinstallato.

È possibile usare il nuovo Edge per installare le app Web insieme Microsoft Store app. Ad esempio, è possibile installare l'app Web Microsoft Office per visualizzare e modificare i file ospitati SharePoint o OneDrive. Per installare l Office app Web, visitare e selezionare il pulsante App Available (Disponibile per l'app) o Install Office (Installa Office https://www.office.com nella barra degli indirizzi).   Selezionare **Installa per** confermare.
...
- Batteria > di sistema: abilitare manualmente la risparmio batteria o impostare una soglia della batteria a quel punto risparmio batteria modalità automatica.
...
>
> - È possibile eseguire nuovamente la calibrazione dei colori Impostazioni ogni volta che si desidera
...
Quando si attiva un collegamento ipertestuale o si apre un tipo di file con più di un'app installata che lo supporta, viene visualizzata una nuova finestra che chiede di selezionare l'app installata che deve gestire il tipo di file o collegamento. In questa finestra è anche possibile scegliere di fare in modo che l'app selezionata gestirà il file o il tipo di collegamento "Once" o "Always".
...
In **Impostazioni** Sistema audio impostare in modo esplicito gli altoparlanti  >    >   **predefiniti (analog Feature Audio Driver)** come **dispositivo predefinito.** HoloLens necessario ricordare questa impostazione anche se il microfono viene rimosso e riconnesso in un secondo momento.
...
Per una AAD predefinita, gli utenti possono ottenere oggi un risultato simile a questo senza questa modifica. AAD dispositivi aggiunti configurati per la modalità tutto schermo possono accedere a un account visitatore con un solo pulsante nella schermata di accesso. Dopo aver eseguito l'accesso all'account visitatore, il dispositivo non richiederà di nuovo l'accesso fino a quando il visitatore non viene disconnesso in modo esplicito dal menu Start o non viene riavviato il dispositivo.
...
Nelle compilazioni precedenti, se un dispositivo ha una configurazione in modalità tutto schermo, che è una combinazione di accesso globale assegnato e accesso assegnato ai membri del gruppo AAD, se la determinazione dell'appartenenza AAD un gruppo non è riuscita, l'utente non visualizza " nulla nel menu[Start](hololens-kiosk.md#issue---no-apps-are-shown-in-start-menu-in-kiosk-mode)".

A partire da questa versione Windows, l'esperienza in modalità tutto schermo verrà fallback alla configurazione globale della modalità tutto schermo (se presente) in caso di errori durante AAD modalità tutto schermo del gruppo.

### <a name="new-settings-uris-for-page-settings-visibility"></a>Nuovi URI Impostazioni per la visibilità Impostazioni pagina

In [Windows Holographic versione 20H2](hololens-release-notes.md#windows-holographic-version-20h2) sono stati aggiunti i criteri [Impostazioni/PageVisibilityList](/windows/client-management/mdm/policy-csp-settings#settings-pagevisibilitylist) per limitare le pagine visibili all'interno dell'app Impostazioni. PageVisibilityList è un criterio che consente agli amministratori IT di impedire che pagine specifiche nell'app System Impostazioni siano visibili o accessibili oppure di eseguire questa operazione per tutte le pagine ad eccezione di quelle specificate.
...
Si sta espandendo l'elenco degli URI Impostazioni disponibili, che gli amministratori IT possono gestire. Alcuni di questi URI sono per le nuove aree disponibili all'interno della nuova app Impostazioni app. Se si usa un Impostazioni/PageVisibilityList, esaminare l'elenco seguente e modificare le pagine consentite o bloccate in base alle esigenze.
...
| Sistema > audio > volume app e preferenze del dispositivo | `ms-settings:apps-volume`                          |
...
| Tastiera & lingua > tastiera                           | `ms-settings:keyboard`                             |
...
Condividere le informazioni con i dispositivi Windows 10, inclusi i PC e altri HoloLens 2 dispositivi. È possibile provare in esperienze condivise **Impostazioni** sistema per condividere file o URL da un HoloLens  >    >   a un PC. Per altri dettagli, leggere altre informazioni su come condividere [elementi con i dispositivi vicini in Windows 10](https://support.microsoft.com/windows/share-things-with-nearby-devices-in-windows-10-0efbfe40-e3e2-581b-13f4-1a0e9936c2d9).
...
✔️ [configurazione delle impostazioni dei criteri per il](#configuring-policy-settings-for-the-new-microsoft-edge)nuovo Microsoft Edge : esaminare le nuove configurazioni disponibili per Microsoft Edge.
...
- Risolve un problema a causa del quale Impostazioni'app si arresta in modo anomalo quando si tenta di modificare una password per un account locale.
...
- I clienti che usano AdvancedPhotoCapture o LowLagPhotoCapture per acquisire foto con HoloLens 2 possono ora recuperare la posizione della fotocamera fino a 3 secondi dopo l'acquisizione della foto.
...
Questo aggiornamento qualitativo mensile non contiene modifiche di cui è necessario fare conto. È quindi possibile provare le build più recenti per Windows Holographic versione 2004.
...
- Il tracciamento delle mani ora mantiene il tracciamento in molti nuovi casi in cui la mano in precedenza sarebbe stata persa.  In alcuni di questi nuovi casi, solo la posizione del palmo continua ad aggiornarsi in base alla mano reale dell'utente, mentre le altre giunzioni vengono dedotto in base a una posizione precedente.  Questa modifica consente di migliorare la coerenza di rilevamento nei movimenti, ad esempio lo slapping, la generazione di eccezioni, l'ambito e l'affondo.  È utile anche nei casi in cui la mano è vicina a una superficie o contiene un oggetto.  Quando vengono dedotto i giunzioni delle mani, il valore di accuratezza [per](/uwp/api/windows.perception.people.jointposeaccuracy?view=winrt-19041&preserve-view=true) giunzione verrà impostato su "Approssimato" anziché su "Alto".
...
- Gli utenti dovrebbero vedere un numero molto inferiore di arresti anomali della Configurazione guidata dopo l'avvio quando avviano ET, Iris dall'app impostazioni, nuovo utente o avviso popup di notifica.
...
| [Avviare automaticamente un'app in modalità tutto schermo con più app](hololens-release-notes.md#automatic-launch-of-an-application-in-multiple-app-kiosk-mode)                | Imposta l'avvio automatico di un'applicazione quando si accede a una modalità tutto schermo con più app.                                                        |
...
In HoloLens 2, le posizioni oculare consentono un posizionamento accurato dell'ologramma, una comoda esperienza di visualizzazione e una migliore qualità dello schermo. Le posizioni degli occhi vengono calcolate internamente come parte del calcolo del tracciamento oculare. Tuttavia, ciò richiede che ogni utente eserciti la calibrazione del tracciamento oculare, anche quando l'esperienza potrebbe non richiedere l'input dello sguardo fisso.
...
L'installazione del certificato attualmente supporta i file con estensione cer e crt. I proprietari dei dispositivi possono installare i certificati nel computer locale e nell'utente corrente.  Tutti gli altri utenti possono eseguire l'installazione solo in Utente corrente. Gli utenti possono rimuovere solo i certificati installati direttamente dall'Impostazioni utente. Se un certificato è stato installato con altri mezzi, deve essere rimosso anche dallo stesso meccanismo.
...
A questo punto, durante la Configurazione HoloLens 2, dopo la connessione con il Wi-Fi, la Configurazione automatica verifica la presenza di un profilo di Autopilot per il dispositivo. Se ne viene trovato uno, verrà usato per completare il resto del AAD join e del flusso di registrazione. In altre parole, l'uso da ethernet a USB-C o da Wi-Fi all'adattatore USB-C non è più un requisito, ma continuano a funzionare se forniti all'inizio della configurazione. Altre informazioni su [Autopilot per HoloLens 2 dispositivi](hololens2-autopilot.md).
...
| MixedReality\AADGroupMembershipCacheValidityInDays | Controlla il numero di giorni Azure AD cache delle appartenenze ai gruppi per la modalità tutto schermo per i gruppi Azure AD utenti. | Vedere qui di seguito.                                                           |
...
- Abilitato l'uso di chioschi in modalità offline con AAD gruppi per un massimo di 60 giorni.
...
1. Consentire Azure AD'accesso dell'utente 1 quando è disponibile Internet, dopo che l'utente ha eseguito l'accesso e Azure AD l'appartenenza al gruppo è stata confermata correttamente, verrà creata la cache.
1. Ora Azure AD'utente 1 può portare HoloLens offline e usarlo per la modalità tutto schermo, purché il valore dei criteri consenta il numero X di giorni.
1. I passaggi 4 e 5 possono essere ripetuti per qualsiasi altro utente di Azure AD N. Punto chiave è che qualsiasi utente di Azure AD deve accedere al dispositivo usando Internet in modo che almeno una volta sia possibile determinare che sono membri del gruppo Azure AD a cui è destinata la configurazione tutto schermo.
...
Questo aggiornamento qualitativo mensile non contiene alcuna modifica di cui si è bisogno. È quindi necessario provare le build più recenti per Windows Holographic, versione 2004.
...
- È stato risolto un problema che causava l HoloLens app per l'arresto anomalo della shell durante l'HoloLens Emulator con accelerazione hardware in determinati dispositivi.
- È stato risolto un problema relativo all'inoltro HTTPS per Windows Portale di dispositivi in HoloLens 2.
...
- Correzione di un bug per impedire l'apertura ripetuta di app immersive Microsoft Edge più volte.
...
L'aggiornamento software principale di maggio 2020 per HoloLens 2, *Windows Holographic, versione 2004* include una serie di nuove interessanti funzionalità, ad esempio il supporto per Windows Autopilot, la modalità app dark, il supporto Ethernet USB per gli hotspot 5G/LTE e molto altro ancora. Per eseguire l'aggiornamento alla versione più recente, aprire l'app Impostazioni, passare a Aggiorna & Sicurezza e selezionare il pulsante Controlla    **** **aggiornamenti.**   
...
Per altre informazioni, [vedere Windows Autopilot per HoloLens 2 di valutazione.](hololens2-autopilot.md)
...
La distribuzione e la gestione di app MDM per le app line-of-business sono fondamentali per HoloLens. Gli amministratori e gli utenti devono visualizzare lo stato di installazione dell'app per il controllo e la diagnosi. In questa versione sono stati aggiunti altri dettagli in Account di accesso Impostazioni aziendale o dell'istituto di istruzione  >    >    >  **Fare clic su Informazioni sull'account.**  >  
...
- Miglioramento della stabilità dell'ologramma nell'acquisizione di realtà mista quando viene usato l'algoritmo *AutoPlanar HolographicDepthReprojectionMethod.*
...
- Miglioramento del rilevamento delle mani in modo che, quando si punta con il dito indice, la parte superiore di tale dito sia meno probabile che si arriccia in modo imprevisto.
...
- Sono state introdotte correzioni di riproduzione dell'ultima fase (LSR). Miglioramento del rendering visivo degli ologrammi in modo da risultare più stabile e nitido, con un'analisi più accurata della loro profondità. Questo sintomo sarà più evidente dopo questo aggiornamento se le app non impostano correttamente la profondità degli ologrammi.
...
- Correzione dei problemi di risoluzione dei problemi relativi alle applicazioni olografiche bloccate in uno stato di sospensione all'avvio fino a quando il menu Start è stato aperto e quindi chiuso.
