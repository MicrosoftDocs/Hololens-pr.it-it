...
- Portale di dispositivi ha metodi migliorati per inviare una notifica al cliente quando Esplora file problemi durante l'apertura di file bloccati.
...
- Le prestazioni di ricarica della batteria sono ottimizzate per aumentare il runtime durante la ricarica con un maggiore caricamento della CPU.
...
- L'utilizzo della batteria per il nuovo Microsoft Edge potrebbe non essere accurato, a causa della sua natura come applicazione desktop Win32 supportata da un livello adattatore UWP (nessuna correzione prevista a breve).
...
1. In **Calibrazione colori visualizzazione** selezionare il pulsante Esegui **calibrazione colori** schermo.
...
Nelle build precedenti, se un dispositivo aveva una configurazione in modalità tutto schermo, che è una combinazione di accesso assegnato a livello globale e accesso assegnato ai membri del gruppo AAD, se la determinazione dell'appartenenza al gruppo AAD non è riuscita, l'utente non visualizza " nulla nel menu[Start](hololens-kiosk.md#kiosk-mode-behavior-changes-for-handling-of-failures)".
...
Leggere il processo completo di [installazione delle app](app-deploy-app-installer.md)in HoloLens 2 con Programma di installazione app .  
...
| [Gestione certificati](hololens-release-notes.md#certificate-manager)   | Consente ai nuovi metodi più semplici di installare e rimuovere i certificati dall Impostazioni app.     |
...
| [Abilitato Impostazioni visibilità della pagina per HoloLens 2](hololens-release-notes.md#enabled-settings-page-visibility-for-hololens-2)      | Criteri per scegliere le pagine da visualizzare nell Impostazioni app.             |
...
Durante la configurazione automatica, dopo aver HoloLens 2 con wi-fi, configurazione automatica verifica la presenza di un profilo Autopilot per il dispositivo. Se ne viene trovato uno, verrà usato per completare il resto del flusso di aggiunta e registrazione di AAD. In altre parole, l'uso di ethernet in USB-C o da Wi-Fi all'adattatore USB-C non è più un requisito, ma continuano a funzionare se forniti all'inizio della configurazione. Altre informazioni su [Autopilot per HoloLens 2 dispositivi](hololens2-autopilot.md).
...
- Risolto un problema che causava HoloLens app che modificano il formato pixel per il rendering nero nella HoloLens Emulator.
- Risolto un problema relativo all'avvio dell'app Foto avvio iniziale dopo l'aggiornamento dalla versione 1903.

## <a name="windows-holographic-version-2004"></a>Windows Holographic, versione 2004  
- Build - 19041.1103

L'aggiornamento software principale di maggio 2020 per HoloLens 2, *Windows Holographic, versione 2004,* include una serie di nuove interessanti funzionalità, ad esempio il supporto per Windows Autopilot, la modalità scura dell'app, il supporto Ethernet USB per gli hotspot 5G/LTE e molto altro ancora. Per eseguire l'aggiornamento alla versione più recente, aprire l'app Impostazioni, passare ad Aggiorna & Security e selezionare il pulsante Controlla    **** **aggiornamenti.**   
...
La distribuzione e la gestione di app MDM per le app line-of-business è fondamentale per HoloLens. Gli amministratori e gli utenti devono visualizzare lo stato di installazione dell'app per il controllo e la diagnosi. In questa versione sono stati aggiunti altri dettagli in Account Impostazioni accesso aziendale o dell'istituto di istruzione  >    >    >  **Fare clic su Informazioni sull'account.**  >  
