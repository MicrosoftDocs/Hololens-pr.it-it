...
[Nuova app Impostazioni app](#new-settings-app) | L'app Impostazioni è stata sostituita da una versione aggiornata con nuove funzionalità e impostazioni. | Utente finale |
...
Il nuovo Microsoft Edge offre agli amministratori IT un set molto più ampio di criteri del browser HoloLens 2 rispetto a quelli disponibili in precedenza con i Microsoft Edge.
...
- Sospensione > di & sistema: scegliere quando il dispositivo deve andare in sospensione dopo un periodo di inattività.
...
- Rete & Internet:
  ...
  - È ora possibile abilitare la modalità aereo in HoloLens 2.
...
- Account > altri utenti: i proprietari dei dispositivi possono aggiungere utenti, aggiornare gli utenti standard ai proprietari dei dispositivi, effettuare il downgrade dei proprietari dei dispositivi agli utenti standard e rimuovere gli utenti.
...
In [Windows Holographic versione 20H2](hololens-release-notes.md#windows-holographic-version-20h2) è stato aggiunto il criterio [Impostazioni/PageVisibilityList](/windows/client-management/mdm/policy-csp-settings#settings-pagevisibilitylist) per limitare le pagine visibili all'interno dell'app Impostazioni. PageVisibilityList è un criterio che consente agli amministratori IT di impedire a pagine specifiche nell'app System Impostazioni di essere visibili o accessibili oppure di eseguire questa operazione per tutte le pagine ad eccezione di quelle specificate.
...
✔️ condividi[elementi con i dispositivi nelle vicinanze:](#share-things-with-nearby-devices)è possibile disabilitare la nuova funzionalità di condivisione nelle vicinanze.
...
| [Gestione certificati](hololens-release-notes.md#certificate-manager)   | Consente ai nuovi metodi più semplici di installare e rimuovere i certificati dall'app Impostazioni semplice.     |
...
| [Abilitata Impostazioni visibilità della pagina per HoloLens 2](hololens-release-notes.md#enabled-settings-page-visibility-for-hololens-2)      | Criteri per scegliere le pagine da visualizzare nell Impostazioni app.             |
...
In HoloLens 2, le posizioni oculare consentono un posizionamento accurato dell'ologramma, una comoda esperienza di visualizzazione e una migliore qualità dello schermo. Le posizioni degli occhi vengono calcolate internamente come parte del calcolo del tracciamento oculare. Tuttavia, ciò richiede che ogni utente possa eseguire la calibrazione del tracciamento oculare, anche quando l'esperienza potrebbe non richiedere l'input dello sguardo fisso.
...
Questo criterio controlla per quanti giorni è consentito Azure AD cache di appartenenza ai gruppi per le configurazioni di accesso assegnato Azure AD gruppi per l'utente connesso. Quando questo valore dei criteri è impostato solo su un valore maggiore di 0, la cache viene usata in caso contrario.  
...
L'aggiornamento software principale di maggio 2020 per HoloLens 2, *Windows Holographic, versione 2004,* include una serie di nuove interessanti funzionalità, ad esempio il supporto per Windows Autopilot, la modalità dark dell'app, il supporto Ethernet USB per gli hotspot 5G/LTE e molto altro ancora. Per eseguire l'aggiornamento alla versione più recente, aprire l'app Impostazioni, passare a Update & Security e selezionare il   pulsante Check for Updates (Verifica **** **aggiornamenti).**   
...
- **Affidabilità del commutatore utente:** Il sistema è ora più veloce e affidabile per aggiornare le dimensioni della mano quando si condivide un dispositivo.
