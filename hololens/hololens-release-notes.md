...
- Correzione di un problema per cui Wi-Fi proxy non può essere salvato quando l'interfaccia utente delle proprietà di Wi-Fi viene avviata da **Impostazioni -> Rete & Internet -> Stato -> Proprietà**.
...
Il Microsoft Edge team rende disponibili tre canali di anteprima per la community di Edge Insider: Beta, Dev e Canary. L'installazione di un canale di anteprima non disinstalla la versione rilasciata di Microsoft Edge nel HoloLens 2 ed è possibile installarne più di una contemporaneamente. 
...
> [!IMPORTANT]
> L'inserimento di **un microfono USB non lo imposta automaticamente come dispositivo di input.** Quando si collega un set di cuffi USB-C, gli utenti osserveranno che l'audio della cuffia verrà reindirizzato automaticamente alle cuffie, ma il sistema operativo HoloLens assegna la priorità all'array del microfono interno rispetto a qualsiasi altro dispositivo di input. **Per usare un microfono USB-C, seguire questa procedura.**
...
In [Windows Holographic versione 20H2](hololens-release-notes.md#windows-holographic-version-20h2) sono stati aggiunti i criteri [Impostazioni/PageVisibilityList](/windows/client-management/mdm/policy-csp-settings#settings-pagevisibilitylist) per limitare le pagine visibili all'interno dell'app Impostazioni app. PageVisibilityList è un criterio che consente agli amministratori IT di impedire a pagine specifiche nell'app System Impostazioni di essere visibili o accessibili oppure di eseguire questa operazione per tutte le pagine ad eccezione di quelle specificate.
...
| System > Sound > Volume app e preferenze dispositivo | `ms-settings:apps-volume`                          |
| System > Sound > Gestire i dispositivi audio              | `ms-settings:sound-devices`                        |
| System > Archiviazione > Configure Archiviazione Sense (Configurare Archiviazione Sense)         | `ms-settings:storagepolicies`                      |
...
✔️ configurazione [delle impostazioni dei criteri per il](#configuring-policy-settings-for-the-new-microsoft-edge)nuovo Microsoft Edge : esaminare le nuove configurazioni disponibili per l'Microsoft Edge.
...
È possibile trovare app di esempio [Windows esempi universali GitHub](https://github.com/microsoft/Windows-universal-samples/tree/master/Samples) testare questo flusso.
...
| [Lunghezza registrazione aumentata](hololens-release-notes.md#recording-length-increased) | Le registrazioni MRC non hanno più un tempo massimo di 5 minuti. |
...
Quando viene visualizzata la schermata principale del provisioning, la Configurazione fuori sede conta fino a 10 secondi prima di avviare automaticamente l'applicazione di tutti i pacchetti di provisioning. Gli utenti possono [comunque confermare o annullare](hololens-provisioning.md#auto-confirm-provisioning-packages-in-oobe) entro questi 10 secondi dopo la verifica dei pacchetti previsti.
...
Ora durante la configurazione della configurazione automatica, dopo HoloLens 2 connessione con Wi-Fi, La configurazione automatica verifica la presenza di un profilo Autopilot per il dispositivo. Se ne viene trovato uno, verrà usato per completare il resto del flusso di aggiunta e registrazione di AAD. In altre parole, l'uso di ethernet da USB-C o da Wi-Fi all'adattatore USB-C non è più un requisito, ma continuano a funzionare se forniti all'inizio della configurazione. Altre informazioni su [Autopilot per HoloLens 2 dispositivi](hololens2-autopilot.md).

### <a name="tenantlockdown-csp-and-autopilot"></a>Tenantlockdown CSP e Autopilot
- Mantiene i dispositivi nel tenant dell'organizzazione bloccandoli al tenant anche tramite reimpostazione o reflash del dispositivo. Con ulteriore sicurezza, non consentire la creazione di account in tramite provisioning. 
...
Una volta che il nodo RequireNetworkInOOBE dei CSP TenantLockdown è impostato su true in HoloLens 2, la Configurazione remota attende per un periodo illimitato che il profilo autopilot sia scaricato e applicato correttamente, dopo la connettività di rete. 
...
1. Ora Azure AD'utente 1 può portare HoloLens offline e usarlo per la modalità tutto schermo, purché il valore dei criteri consenta un numero X di giorni. 
...
Questi criteri di aggiornamento sono ora abilitati nei HoloLens 2 seguenti:
...
In modalità di ricerca, l'HoloLens 2 diventa uno strumento potente per la ricerca di visione computer. Rispetto alle edizioni precedenti, la modalità di ricerca per HoloLens 2 presenta i vantaggi seguenti:
...
I ricercatori hanno ora la possibilità di abilitare la modalità di ricerca nei dispositivi HoloLens per accedere a tutti questi flussi esterni di sensori di immagini non elaborati rivolte verso l'esterno. La modalità di HoloLens 2 fornisce anche l'accesso alle letture di accelerometro, giroscopio e magnetometro. Per proteggere la privacy degli utenti, le immagini non elaborate della fotocamera per il tracciamento oculare non sono disponibili tramite la modalità di ricerca, ma la direzione dello sguardo è disponibile tramite le API esistenti.
...
- È stato risolto un problema che impediva la visualizzazione di un dispositivo HoloLens in Esplora file tramite Media Transfer Protocol (MTP) quando il dispositivo è configurato come un chiosco multimediale a [app singola.](hololens-kiosk.md) Si noti che MTP (e la connessione USB in generale) possono comunque essere disabilitati usando il [criterio AllowUSBConnection.](/windows/client-management/mdm/policy-csp-connectivity#connectivity-allowusbconnection)
...
- Gli sviluppatori possono ora scegliere tra abilitare o disabilitare la Portale di dispositivi richiede una connessione sicura.
...
- Correzione di un problema che causava l'arresto anomalo HoloLens di app esclusive nella shell durante l'uso del HoloLens Emulator con accelerazione hardware in determinati dispositivi.
- È stato risolto un problema relativo all'inoltro HTTPS per Windows Portale di dispositivi in HoloLens 2.
...
- Quando si accede Portale di dispositivi tramite una connessione Wi-Fi, un Web browser potrebbe impedire l'accesso a a causa di un certificato non valido. Il browser potrebbe segnalare un errore, ad esempio "ERR_SSL_PROTOCOL_ERROR", anche se il certificato del dispositivo è stato precedentemente considerato attendibile. In questo caso, non è possibile Portale di dispositivi, perché non è possibile ignorare gli avvisi di sicurezza. Questo aggiornamento ha risolto il problema. Se il certificato del dispositivo è stato scaricato in precedenza e considerato attendibile in un PC per rimuovere gli avvisi di sicurezza del browser e si verifica l'errore SSL, il nuovo certificato deve essere scaricato e considerato attendibile per risolvere gli avvisi di sicurezza del browser.
...
- È stato risolto un problema che causava HoloLens app che modificano il formato pixel per il rendering del nero nell'emulatore HoloLens.
...
- Risolto un problema che causava HoloLens app che modificano il formato pixel per il rendering del nero nel HoloLens Emulator.
...
Le chiavi di sicurezza FIDO2 sono un metodo di autenticazione senza password basato su standard "non phishable" che può essere in qualsiasi fattore di forma. FIDO è uno standard aperto per l'autenticazione senza password. Consente a utenti e organizzazioni di accedere alle proprie risorse senza un nome utente o una password. Usano invece una chiave di sicurezza esterna o una chiave della piattaforma incorporata in un dispositivo.
...
- **Riduzione dei tocchi d'aria accidentali:** È stato migliorato il rilevamento del movimento del tocco dell'aria. Sono ora disponibili meno attivazioni accidentali in diversi scenari comuni, ad esempio quando si rilasciano le mani ai lati.
...
È ora possibile usare i comandi vocali con qualsiasi app nel dispositivo. Per altre informazioni, vedere [Usare la voce per HoloLens](hololens-cortana.md). Vedere anche [Lingue supportate per HoloLens 2](hololens2-language-support.md).  
...
- È stato risolto un problema a causa del quale un driver si arresta in modo anomalo durante la calibrazione oculare.
...
- Correzione dei problemi relativi alle applicazioni olografiche bloccate in uno stato di sospensione all'avvio fino all'apertura e alla chiusura menu Start'applicazione.
