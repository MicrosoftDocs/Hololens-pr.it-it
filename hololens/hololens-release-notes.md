...
- Se l'account OneDrive ha problemi (ad esempio dopo  la modifica della password), nella pagina Impostazioni correzione.
...
>[!IMPORTANT]
>Per eseguire l'aggiornamento a questa build, HoloLens 2 i dispositivi devono eseguire l'aggiornamento di febbraio 2021 (build 19041.1136) o versione più recente. Se l'aggiornamento della funzionalità non è disponibile, aggiornare prima il dispositivo e riprovare.
...
>
> Con l'introduzione Windows Holographic versione 21H1, vengono sospesi gli aggiornamenti mensili di manutenzione per Windows **Holographic versione 1903.** In questo modo è possibile concentrarsi sulle versioni più recenti e continuare a offrire miglioramenti importanti.
...
> [!IMPORTANT]
> A causa del volume di criteri del browser supportati dal nuovo Microsoft Edge, il team non è in grado di garantire che ogni nuovo criterio funzioni HoloLens 2. Tuttavia, sono stati testati e confermati rispetto al nuovo Microsoft Edge equivalente di ogni criterio di Microsoft Edge legacy supportato in precedenza HoloLens 2 funzionano come previsto. Vedere [Versione legacy di Microsoft Edge per Microsoft Edge mapping](/deployedge/microsoft-edge-policy-map-legacy-to-newedge) dei criteri per trovare il nuovo Microsoft Edge equivalente di ogni criterio Microsoft Edge browser legacy in uso con HoloLens 2.
>
...
Il team Microsoft Edge rende disponibili tre canali di anteprima per la community di Edge Insider: Beta, Dev e Canary. L'installazione di un canale di anteprima non disinstalla la versione rilasciata di Microsoft Edge nel HoloLens 2 ed è possibile installarne più di uno contemporaneamente. 
...
Nelle build precedenti, se un dispositivo ha una configurazione tutto schermo, ovvero una combinazione di accesso assegnato a livello globale e accesso assegnato ai membri del gruppo AAD, se la determinazione dell'appartenenza al gruppo AAD non è riuscita, l'utente visualizza " nulla visualizzato nel menu[Start](hololens-kiosk.md#kiosk-mode-behavior-changes-for-handling-of-failures)".
...
### <a name="new-settings-uris-for-page-settings-visibility"></a>Nuovi IMPOSTAZIONI per la visibilità Impostazioni pagina
...
Se si visita [Visibilità Impostazioni](settings-uri-list.md)pagina , è possibile trovare le istruzioni per usare questo provider di servizi di configurazione e l'elenco degli URI disponibili nelle versioni precedenti.
...
>
> Una pagina delle impostazioni è deprecata in queste build più nuove. La vecchia **pagina & proxy Internet** non è più disponibile come impostazione  >   globale. Le nuove impostazioni del proxy per ogni connessione sono disponibili in **Proprietà** Wi-Fi Di rete & Rete o &  >    >   **Proprietà**  >  **Ethernet**  >  **Internet**.
...
| Rete & internet > modalità Aereo                   | `ms-settings:network-airplanemode`                 |
...
A questo Impostazioni App, un utente può configurare il comportamento di [Diagnostica di fallback.](hololens-diagnostic-logs.md) Nell'app Impostazioni passare alla **pagina Risoluzione**  ->  **dei problemi** relativi alla privacy per configurare questa impostazione.
...
Oltre ai precedenti strumento di risoluzione dei problemi all'interno dell'app Impostazioni, è stato aggiunto un nuovo strumento di risoluzione dei problemi con l'aggiunta della nuova app Impostazioni per gli aggiornamenti del sistema operativo. Passare **a** Impostazioni  ->  **Risoluzione dei problemi &amp; di Windows**  >    >  **e** selezionare **Avvia.** In questo modo è possibile raccogliere tracce durante la riproduzione del problema con gli aggiornamenti del sistema operativo per facilitare la risoluzione dei problemi con l'IT o il supporto tecnico.
...
Se attualmente si usa [Impostazioni visibilità,](settings-uri-list.md) è possibile apportare modifiche agli URI esistenti consentiti o bloccati.
...
✔️ [configurazione delle impostazioni dei criteri per il nuovo Microsoft Edge](#configuring-policy-settings-for-the-new-microsoft-edge): esaminare le nuove configurazioni disponibili per Microsoft Edge.
...
- Correzione di una perdita di memoria nel servizio Portale di dispositivi, il problema ha causato un aumento dell'utilizzo della memoria da parte del servizio che ha causato l'errore di allocazione della memoria da parte di altre applicazioni.
...
1.  Assicurarsi di aver eseguito l'accesso al HoloLens 2 dispositivo
...
- Quando si avvia ET, Iris dall'app delle impostazioni, un nuovo utente o un avviso popup di notifica, gli utenti dovrebbero vedere meno arresti anomali della configurazione guidata dopo l'avvio.
...
| [Nuovi criteri di restrizione dei dispositivi per HoloLens 2](hololens-release-notes.md#new-device-restriction-policies-for-hololens-2)       | Criteri di gestione dei dispositivi abilitati di recente per HoloLens 2.                                                                                |
...
In HoloLens 2, le posizioni oculare consentono un posizionamento accurato dell'ologramma, un'esperienza di visualizzazione comoda e una migliore qualità dello schermo. Le posizioni degli occhi vengono calcolate internamente come parte del calcolo del tracciamento oculare. Tuttavia, ciò richiede che ogni utente eserciti la calibrazione del tracciamento oculare, anche quando l'esperienza potrebbe non richiedere l'input dello sguardo fisso.
...
|:-------------------|:-----------------|:-----------------------------------|
| App non abilitata per lo sguardo o Holographic Shell |Viene visualizzata la finestra di dialogo di richiesta di calibrazione del tracciamento oculare. | Non viene visualizzato alcun prompt. |
...
1. Passare a **Impostazioni'app > certificati di aggiornamento e sicurezza > certificati**.
...
Altre informazioni sul [provisioning di avvio automatico da usb](hololens-provisioning.md#auto-launch-provisioning-from-usb) sono disponibili nella documentazione HoloLens sul provisioning automatico.
...
1. [Flash del HoloLens 2](hololens-insider.md#ffu-download-and-flash-directions) alla [build 19041.1361 o versione più recente.](https://aka.ms/hololens2previewdownload) 
...
- È stata rimossa la necessità di adattatori USB-C per ridurre le esigenze hardware grazie all'abilitazione di Autopilot nei dispositivi Wi-Fi connessi.
...
- Mantiene i dispositivi nel tenant dell'organizzazione bloccandoli al tenant anche tramite la reimpostazione o la reflash del dispositivo. Con un'ulteriore sicurezza, non è consentita la creazione dell'account in tramite provisioning. 
...
|     [EnergySaverBatteryThresholdPluggedIn](/windows/client-management/mdm/policy-csp-power#power-energysaverbatterythresholdpluggedin)     |     Valore di esempio da usare in Progettazione Windows, ad esempio 100                                                                          |
|     [StandbyTimeoutOnBattery](/windows/client-management/mdm/policy-csp-power#power-standbytimeoutonbattery)                  |     Valore di esempio da usare in Progettazione Windows, ad esempio`<enabled/><data   id="EnterDCStandbyTimeOut" value="100"/>`          |
|     [StandbyTimeoutPluggedIn](/windows/client-management/mdm/policy-csp-power#power-standbytimeoutpluggedin)                  |     Valore di esempio da usare in Progettazione Windows, ad esempio`<enabled/><data   id="EnterACStandbyTimeOut" value="100"/>`           |
...
Per informazioni dettagliate su questi criteri di aggiornamento e su come usarli per i dispositivi HoloLens, vedere Gestire gli [aggiornamenti HoloLens.](hololens-updates.md)
...
In modalità di ricerca, il HoloLens 2 diventa uno strumento potente per la ricerca di visione computer. Rispetto alle edizioni precedenti, la modalità di ricerca HoloLens 2 offre i vantaggi seguenti:
...
I ricercatori hanno ora la possibilità di abilitare la modalità di ricerca nei dispositivi HoloLens per accedere a tutti questi flussi esterni di sensori di immagini non elaborati. Modalità di ricerca HoloLens 2 consente anche l'accesso alle letture di accelerometro, giroscopio e magnetometro. Per proteggere la privacy degli utenti, le immagini della fotocamera con tracciamento oculare non sono disponibili tramite la modalità di ricerca, ma la direzione dello sguardo è disponibile tramite le API esistenti.
...
> [!NOTE]
> Il HoloLens userà la durata predefinita della registrazione video (5 minuti) se si verifica una delle condizioni seguenti:
...
- Correzione di un problema dovuto alla memorizzazione nella cache HTTP che interferisce con la modalità tutto schermo destinata Azure AD gruppi.
...
Questo aggiornamento qualitativo mensile non contiene alcuna modifica di cui si può fare conto. È quindi possibile provare le build più recenti per Windows Holographic, versione 2004.
...
- Questa versione include la correzione di rilevamento del proxy NCSI per risolvere i problemi di rilevamento Internet su un proxy di rete. NCSI può usare il proxy del computer e il proxy per profilo per il rilevamento della connettività Internet. Il proxy per utente sarà supportato da NCSI nella versione futura.
...
- Gli sviluppatori possono ora scegliere se abilitare o disabilitare la Portale di dispositivi richiede una connessione sicura.
...
L'aggiornamento software principale di maggio 2020 per HoloLens 2, *Windows Holographic, versione 2004* include una serie di nuove interessanti funzionalità, ad esempio il supporto per Windows Autopilot, la modalità app scura, il supporto Ethernet USB per gli hotspot 5G/LTE e molto altro ancora. Per eseguire l'aggiornamento alla versione più recente, aprire l'app Impostazioni, passare ad Aggiorna & Sicurezza e selezionare il pulsante Controlla    **** **aggiornamenti.**   
...
Le chiavi di sicurezza FIDO2 sono un metodo di autenticazione senza password basato su standard "unphishable" che può essere utilizzato in qualsiasi fattore di forma. FIDO è uno standard aperto per l'autenticazione senza password. Consente a utenti e organizzazioni di accedere alle proprie risorse senza un nome utente o una password. Usano invece una chiave di sicurezza esterna o una chiave della piattaforma incorporata in un dispositivo.
...
Molte Windows ora supportano le modalità scura e leggera. HoloLens 2 gli utenti possono scegliere la modalità predefinita per le app che supportano entrambi. Dopo l'aggiornamento, la modalità app predefinita è "scura", ma è possibile modificare facilmente questa impostazione: Passare **Impostazioni** Colori di sistema Scegliere la  >    >    >  **modalità app predefinita.** 
...
- Correzione della funzionalità **di Selezione** comandi vocali durante la configurazione iniziale per en-CA e en-AU.
