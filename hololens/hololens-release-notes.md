...
---
# <a name="hololens-2-release-notes"></a>Note sulla versione di HoloLens 2
...
>[!NOTE]
> Siamo contenti di iniziare di nuovo a eseguire la versione di anteprima delle nuove funzionalità per i windows insider. Per gli aggiornamenti più recenti, il canale dev verrà in esecuzione sul Canale di sviluppo. Man mano che aggiungiamo altre funzionalità e aggiornamenti alle build di Windows Insider, continueremo a utilizzare le note di [**HoloLens Insider.**](hololens-insider.md) Prepararsi a combinare questi aggiornamenti nella realtà.
...
- I clienti che usano AdvancedPhotoCapture o LowLagPhotoCapture per acquisire foto con HoloLens 2 ora sono in grado di recuperare la posizione della fotocamera fino a 3 secondi dopo l'acquisizione della foto.
...
- È stato risolto un problema per cui gli utenti registrati all'implementazione a fasi non sono in grado di accedere al dispositivo.
...
### <a name="install-apps-on-hololens-2-via-app-installer"></a>Installare app in HoloLens 2 tramite il programma di installazione app
...
- Il tracciamento della mano ora mantiene il tracciamento in molti nuovi casi in cui la mano in precedenza sarebbe stata persa.  In alcuni di questi nuovi casi, solo la posizione del palmo continua ad aggiornarsi in base alla mano reale dell'utente, mentre le altre giunzioni vengono dedote in base a una posizione precedente.  Questa modifica consente di migliorare la coerenza di tracciabilità nei movimenti, ad esempio schiaffi, lancio, ambito e clapping.  È utile anche nei casi in cui la mano si trova vicino a una superficie o in cui è in possesso di un oggetto.  Quando vengono dedote le giunzioni della mano, il valore di accuratezza [per](https://docs.microsoft.com/uwp/api/windows.perception.people.jointposeaccuracy?view=winrt-19041&preserve-view=true) giunzione verrà impostato su "Approximate" invece che su "High".
...
- Gli utenti dovrebbero vedere meno arresti anomali della Configurazione guidata dopo l'avvio durante l'avvio di ET, Iris dall'app impostazioni, nuovo utente o avviso popup di notifica.
...
| [Avviare automaticamente un'app in chiosco multimediale con più app](hololens-release-notes.md#automatic-launch-of-an-application-in-multiple-app-kiosk-mode)                | Imposta l'avvio automatico di un'applicazione quando si accede in modalità tutto schermo con più app.                                                        |
...
### <a name="auto-eye-position-support"></a>Supporto per Posizione automatica degli occhi
...
Queste informazioni sono disponibili in seguito con [altre informazioni sulla calibrazione.](hololens-calibration.md#auto-eye-position-support) 
...
-   **Convalida:** La verifica che un certificato si adequa allo scopo previsto ed è funzionale, può risparmiare tempo significativo, in particolare in ambienti commerciali prima di distribuire certificati su larga scala.
...
L'installazione dei certificati attualmente supporta i file con estensione cer e crt. I proprietari dei dispositivi possono installare certificati in Computer locale e Utente corrente;  tutti gli altri utenti possono eseguire l'installazione solo nell'utente corrente. Gli utenti possono rimuovere solo i certificati installati direttamente dall'interfaccia utente impostazioni. Se un certificato è stato installato con altri mezzi, deve essere rimosso anche dallo stesso meccanismo.
...
### <a name="auto-launch-provisioning-from-usb"></a>Provisioning ad avvio automatico da USB
...
### <a name="auto-confirm-provisioning-packages-in-oobe"></a>Conferma automatica dei pacchetti di provisioning nella Configurazione automatica
...
Quando viene visualizzata la schermata principale del provisioning, la Configurazione fuori sede conta per 10 secondi prima di iniziare automaticamente ad applicare tutti i pacchetti di provisioning. Gli utenti possono [comunque confermare o annullare](hololens-provisioning.md#auto-confirm-provisioning-packages-in-oobe) entro questi 10 secondi dopo aver verificato i pacchetti previsti.
...
1. [Creare un pacchetto di provisioning](hololens-provisioning.md) tramite Progettazione configurazione di [Windows.](https://www.microsoft.com/store/productId/9NBLGGH4TX22) 
...
### <a name="using-autopilot-with-wi-fi-connection"></a>Utilizzo di Autopilot con Wi-Fi connessione
...
Ora durante la configurazione automatica, dopo aver connesso HoloLens 2 con il Wi-Fi, la configurazione automatica controlla la presenza di un profilo Autopilot per il dispositivo. Se ne viene trovato uno, verrà usato per completare il resto del flusso di aggiunta e registrazione di AAD. In altre parole, l'uso di ethernet su USB-C o Wi-Fi per l'adattatore USB-C non è più un requisito, ma continuano a funzionare se forniti all'inizio della Configurazione fuori rete. Altre informazioni su [Autopilot per dispositivi HoloLens 2.](hololens2-autopilot.md)
...
#### <a name="how-to-set-this-using-intune"></a>Come impostarlo usando Intune? 
...
#### <a name="how-to-unset-tenantlockdowns-requirenetworkinoobe-on-hololens-2-using-intune"></a>Come si annulla RequireNetworkInOOBE di TenantLockdown in HoloLens 2 usando Intune? 
...
#### <a name="what-would-happen-during-oobe-if-autopilot-profile-is-unassigned-on-a-hololens-after-tenantlockdown-was-set-to-true"></a>Cosa succederebbe durante la Configurazione guidata, se il profilo di Autopilot non viene assegnato a HoloLens dopo che TenantLockdown è stato impostato su true? 
...
### <a name="cache-azure-ad-group-membership-for-offline-kiosk"></a>Memorizzare nella cache l'appartenenza ai gruppi di Azure AD per chiosco offline
...
1. Creare un profilo di configurazione del dispositivo per il chiosco multimediale per i gruppi di Azure AD e assegnarlo ai dispositivi HoloLens. 
...
- Consente agli utenti di gestire specifici criteri di gestione dei dispositivi, ad esempio bloccando l'aggiunta o la rimozione di pacchetti di provisioning.
...
### <a name="enabled-settings-page-visibility-for-hololens-2"></a>Visibilità della pagina Impostazioni abilitate per HoloLens 2
...
Grazie al feedback dei clienti, abbiamo aumentato la durata della registrazione delle acquisizioni [in realtà mista.](holographic-photos-and-videos.md) Le acquisizioni in realtà mista non saranno più limitate a 5 minuti per impostazione predefinita, ma calcoleranno invece la lunghezza massima di registrazione in base allo spazio disponibile su disco. Il dispositivo stima la durata massima della registrazione video in base allo spazio su disco disponibile fino all'80% dello spazio su disco totale.
...
- Per ulteriori informazioni, vedere l'informativa sulla privacy più recente online.
...
- È stato risolto un problema di configurazione proxy per la connessione VPN.
...
- È stato rimosso un controllo non necessario che ha causato un errore del sistema di runtime.
...
- Maggiore affidabilità del tracciamento delle mani che comporta un minor numero di perdite di tracciamento in scenari specifici.
...
- È stata **** aggiunta un'impostazione in  >  ****  >  **Ologrammi** di sistema delle impostazioni che consente agli utenti di rimuovere automaticamente tutti gli ologrammi dalla casa della realtà mista quando il dispositivo viene arrestato.
...
Per iniziare, vedere [Abilitare l'accesso con chiave](https://docs.microsoft.com/azure/active-directory/authentication/howto-authentication-passwordless-security-key)di sicurezza senza password.

### <a name="improved-mdm-enrollment-via-provisioning-package"></a>Registrazione MDM migliorata tramite pacchetto di provisioning

I pacchetti di provisioning consentono di impostare la configurazione di HoloLens tramite un file di configurazione anziché tramite l'esperienza predefinita di HoloLens. In precedenza, i pacchetti di provisioning dovevano essere copiati nella memoria interna di HoloLens. Ora possono essere su un'unità USB in modo che siano più facili da riutilizzare in più dispositivi HoloLens ed è possibile effettuare il provisioning dei dispositivi in parallelo. I pacchetti di provisioning ora supportano anche un campo per la registrazione nella gestione dei dispositivi, quindi non esiste alcuna configurazione manuale dopo il provisioning.
...
### <a name="line-of-business-application-install-status"></a>Stato di installazione delle applicazioni line-of-business
...
### <a name="expanded-usb-ethernet-support-for-5glte-tethered-devices"></a>Supporto Ethernet USB espanso per dispositivi con tethering 5G/LTE
...
### <a name="hand-tracking-improvements"></a>Miglioramenti al tracciamento della mano
...
Molte app di Windows ora supportano sia le modalità scura che la modalità chiaro. Gli utenti di HoloLens 2 possono scegliere la modalità predefinita per le app che supportano entrambi. Dopo l'aggiornamento, la modalità app predefinita è "scura", ma **** puoi facilmente modificare questa impostazione: passa a Impostazioni colori di sistema  >  ****  >  ****  >  **Scegli la modalità predefinita dell'app.** 
...
- È stato risolto un bug in cui in alcuni casi l'utente corretto non sarebbe stato visualizzato nella schermata di accesso quando si riprendeva una sessione.
...
Molte app di Windows supportano sia la modalità scura che la modalità light. I clienti di HoloLens 2 ora possono scegliere la modalità predefinita per le app che supportano entrambe le combinazioni di colori. In base al feedback dei clienti, la modalità predefinita dell'app viene impostata su "scura", ma puoi facilmente modificare questa impostazione in qualsiasi momento: passa a Impostazioni **> Colori >** sistema per trovare "Scegli la modalità predefinita **dell'app".**
...
**Miglioramenti e correzioni anche nell'aggiornamento:** 
- Assicurati che le sovrimpressione della shell siano incluse nelle acquisizioni in realtà mista.
- Gli sviluppatori irreali ora possono usare la pagina Visualizzazione 3D in Device Portal per testare ed eseguire il debug delle applicazioni.
...
- È stato risolto l'errore "Classe API WinRT IStreamSocketListener non registrata" nelle app ARM a 32 bit.
...
- Miglioramenti e correzioni per la conformità al runtime OpenXR per HoloLens 2 e l'emulatore.
