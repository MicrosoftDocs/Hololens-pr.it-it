...
- [Schermata blu dopo l'annullamento della registrazione dall'anteprima insider in un dispositivo flash con una build Insider](#blue-screen-after-unenrolling-from-insider-preview-on-a-device-flashed-with-an-insider-build)
...
> [!NOTE]
> È disponibile una versione più recente di Remote Assist che include una correzione per questo problema. Aggiornare [i Remote Assist](holographic-store-apps.md#update-apps) alla versione più recente per evitare questo problema.

> [!NOTE]
> A causa della gravità di questo problema noto, è stata temporaneamente sospesa la disponibilità di Windows Holographic, versione 21H1. La build 21H1 è ora nuovamente disponibile, quindi i dispositivi possono essere ancora una volta aggiornati alla build 21H1 più recente.
...
Se le chiamate hanno una durata di 20 minuti e si verifica questo problema, provare a riavviare il dispositivo. Il riavvio del dispositivo tra Remote Assist chiamate aggiorna il dispositivo e lo riporta a uno stato valido.
...
Alcuni clienti hanno rilevato che, durante il tentativo di caricare o scaricare file, l'operazione potrebbe sembrare bloccarsi e quindi verificarsi un timeout o non essere mai completata. Questo problema è separato dal problema noto["file](#downloading-locked-files-doesnt-error) bloccato", che interessa le build di Windows Holographic, versioni 2004, 20H2 e 21H1. Il problema è stato causato da un bug nella gestione di determinate richieste di Portale di dispositivi e viene raggiunto più costantemente quando si usa https, che è l'impostazione predefinita.
...
- Dopo aver abilitato la modalità sviluppatore Impostazioni, l'attivazione del Portale di dispositivi potrebbe richiedere alcuni secondi.
...
Se la visiera ha impronte digitali o sbavature, usare il panno di pulizia microfibra fornito con il HoloLens per pulire la visiera in modo gentile.
...
In caso di problemi di [associazione di un dispositivo Bluetooth,](hololens-connect-devices.md)provare a eseguire le operazioni seguenti:
...
HoloLens 2 supporta il profilo audio Bluetooth A2DP per la riproduzione stereo. Il Bluetooth hands free che consente l'acquisizione del microfono da una Bluetooth periferica non è supportato HoloLens 2.
...
È possibile associare altri Bluetooth HID e GATT con il HoloLens. Tuttavia, potrebbe essere necessario installare le app complementari corrispondenti da Microsoft Store per usare effettivamente i dispositivi.
