...
ms.locfileid: "130351703"
---
# <a name="windows-autopilot-for-hololens-2"></a>Windows Autopilot per HoloLens 2

## <a name="overview"></a>Panoramica

Per distribuire su larga scala, è consigliabile iniziare a usare Windows Autopilot. È considerato "low touch" in quanto semplifica notevolmente la configurazione HoloLens per gli utenti IT e finali. 

A livello generale, un amministratore IT crea in genere le configurazioni pronte per l'azienda e HoloLens 2 dispositivi nei portali MDM. Quando i dispositivi HoloLens 2 vengono avviati con un'esperienza predefinita e si connette a Internet, le configurazioni business-ready per i dispositivi HoloLens 2 registrati vengono scaricate e applicate automaticamente per rendere i dispositivi pronti per l'azienda senza alcun intervento dell'utente.
...
1. Aggiungere il dispositivo a Azure Active Directory (Azure AD). Autopilot per HoloLens non supporta l'aggiunta ad Active Directory o l'aggiunta Azure AD ibrida.
...
Per verificare la versione della build nel dispositivo o il reflash al sistema operativo più recente, usare le istruzioni [ARC (Advanced Recovery Companion)](https://www.microsoft.com/p/advanced-recovery-companion/9p74z35sfrs8?rtc=2&activetab=pivot:overviewtab) e le istruzioni di [reflash del dispositivo.](hololens-recovery.md) I dispositivi recapitati fino alla fine di settembre 2020 Windows holographic versione 1903 preinstallati. Contattare il rivenditore per assicurarsi che i dispositivi pronti per Autopilot siano spediti all'utente.
...
1. [Nell Microsoft Endpoint Manager di amministrazione](https://endpoint.microsoft.com)selezionare Dispositivi Windows Windows registrazione e quindi selezionare Dispositivi Importa in  >    >  Programma   >   **Windows Autopilot Deployment dispositivi.**
...
La pagina dello stato della registrazione (ESP) visualizza lo stato del processo di configurazione completo del dispositivo che viene eseguito quando un utente gestito da MDM accede a un dispositivo per la prima volta. Assicurarsi che la configurazione esp sia simile alla seguente e verificare che le assegnazioni siano corrette.  
...
## <a name="windows-autopilot-for-hololens-2-user-experience"></a>Windows Autopilot per l'HoloLens 2 utente
...
### <a name="issue---mdm-enrollment-fails-with-error-0x80180014-error-code-during-autopilot"></a>Problema: la registrazione MDM ha esito negativo con codice 0x80180014 errore durante Autopilot.

 **Sintomi**
 
Si tratta di un errore visualizzato durante il processo di Autopilot nel dispositivo. Questo problema si applica solo quando un HoloLens dispositivo ha eseguito le operazioni seguenti:

1. Autopilot è già stato passato almeno una volta.
1. È ora in corso la reimpostazione e il nuovo utilizzo per Autopilot.

L'esperienza è autopilot avrà esito negativo con un errore specifico.
...
1. Passare a **Impostazioni** aggiornamento & sicurezza & ripristino e  ->    ->   selezionare **Introduzione.**
    1. In caso di problemi con i passaggi 2 & 3, vedere alternative alla reimpostazione del dispositivo in [Reset/Reflash HoloLens](hololens-recovery.md).

AutoPilot dovrebbe quindi eseguire correttamente la registrazione.

### <a name="issue---autopilot-experience-did-not-start-even-though-the-autopilot-profile-is-assigned-in-intune"></a>Problema: l'esperienza di Autopilot non è stata avviata anche se il profilo Autopilot è assegnato in Intune.
...
1. Verificare innanzitutto che il profilo di Autopilot sia visualizzato come assegnato nel portale MDM, ad esempio Intune. 
1. Riavviare il dispositivo e riprovare. Per altre informazioni, vedere [Problemi noti e limitazioni](hololens2-autopilot.md#troubleshooting).

### <a name="helpful-resources"></a>Risorse utili

Gli articoli seguenti possono essere una risorsa utile per ottenere altre informazioni e risolvere i problemi di Autopilot, ma questi articoli si basano su Windows 10 Desktop e non tutte le informazioni possono essere applicabili a HoloLens:
